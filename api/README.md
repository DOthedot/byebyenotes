...
| A full pull is **paged**, `PULL_LIMIT` (200) at a time | An account holds more rows than any one page: `push` upserts without trimming, and the client's `saveSnapshot` evicts its oldest note **without tombstoning it** — only an explicit delete tombstones — so every note a device ever made is still a row. `pull` returns a `nextCursor` whenever a page comes back full and the client walks them all. The cursor is a keyset over `(updated_at_ms, client_nid)`; both halves are needed, because one push writes a whole batch under a single `now()` and those rows share a timestamp. The **same millisecond expression** must appear in the cursor, the comparison and the `ORDER BY` — an earlier version compared a millisecond-rounded boundary against the raw microsecond column, and half of all batches silently vanished from the pull. |
